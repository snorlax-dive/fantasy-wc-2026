...
export async function GET() {
  ...
  const { data: prof } = await admin.from('profiles').select('team_name, display_name, crest').eq('id', user.id).maybeSingle()
  ...
  let topPts = 0
  if (squad?.id) {
    const { data: sps } = await admin.from('squad_players').select('player_id').eq('squad_id', squad.id)
    ...
    if (bestId != null) {
      const { data: p } = await admin.from('players').select('name').eq('id', bestId).maybeSingle()
  ...
  const Big = ({ value, label }: { value: string | number; label: string }) => (
