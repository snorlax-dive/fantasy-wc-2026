...
const PRICE_BASE: Record<Pos, number> = { GK: 5, DEF: 5, MID: 5.5, FWD: 6 }
function priceFor(pos: Pos, strength: number, apiPlayerId: number): number {
  const jitter = (apiPlayerId % 7) / 7 // deterministic 0..~0.86 so re-seeds are stable
  return Math.round((PRICE_BASE[pos] + strength * 4 + jitter) * 2) / 2 // nearest 0.5
}
